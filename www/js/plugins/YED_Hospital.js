 ...
 *
 * @param HP Price
 * @desc HPを1回復するためにかかる費用
 * @default 10
 *
 * @param MP Price
 * @desc MPを1回復するためにかかる費用
 * @default 20
 *
 * @param State Price
 * @desc 状態異常を回復するためにかかる費用
 * @default 100
 *
 * @param [Visual Setting]
 * @default
 ...
 * @param Nurse Message
 * @desc ナースの挨拶のセリフ
 * @default こんにちは、今日はどうされましたか？
 *
 * @param Heal One Help
 * @desc 個別回復のヘルプウィンドウに表示されるテキスト
 * @default メンバーを個別に回復させます
 *
 * @param Heal All Help (Treat)
 * @desc 全員回復のヘルプウィンドウに表示されるテキスト
 * @default %1Gを払ってメンバー全員を回復させます
 *
 * @param Heal All Help (Healthy)
 * @desc 全員が回復不要な際、全員回復のヘルプウィンドウに表示されるテキスト
 * @default 回復が必要なメンバーが居ません
 *
 * @param Exit Help
 * @desc 退出時のヘルプウィンドウに表示されるテキスト
 * @default 外に出る
 *
 * @param Actor Help (Treat)
 * @desc 回復が必要なアクター選択時のヘルプウィンドウに表示されるテキスト
 * @default %1は回復が必要だ
 *
 * @param Actor Help (Healthy)
 * @desc 回復が不要なアクター選択時のヘルプウィンドウに表示されるテキスト
 * @default %1は回復の必要はない
 *
 * @param Heal One Command
 * @desc 個別回復のコマンドテキスト
 ...
 *
 * @param Text Alignment
 * @desc コマンドウィンドウでのテキストの表示位置を調整します
 * left(左)     center(中央)     right(右)
 * @default center
 *
 * @help
 * 以下は、イベントで使えるプラグインコマンドです。
 *
 * Plugin Command:
 *   OpenHospital      →フィールドから病院のシーンを開きます
 */
