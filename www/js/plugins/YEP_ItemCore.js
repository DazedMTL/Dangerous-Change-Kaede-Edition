...
/*:
* @plugindesc v1.04 Changes the way Items are handled for your game
...
*/
/*:ja
* @plugindesc ゲーム内やアイテムシーン内でのアイテムの処理を変更します
* @author Yanfly Engine Plugins
*
* @param ---一般---
* @default
...
* ============================================================================
*
* このプラグインは、ゲームそのもの、そして
* ゲーム内でのアイテムの扱われ方にいくつかの変更を行います。
*
* 
* １．個別アイテム
...
Game_Actor.prototype.initIndependentEquips = function (equips) {
  ...
  for (var j = 0; j < equips.length; j++) {
    ...
    if (j < maxSlots) {
      ...
      if (item && item.etypeId !== etypeId) continue;
      if (DataManager.isIndependent(item)) {
        if (this.canEquip(item)) {
          var array = $gameParty.gainItem(item, 1);
        ...
        this._equips[j].setEquip(slots[j] === 1, itemId);
      }
    }
  }
  ...
  if (amount > 0) {
    ...
    for (var i = 0; i < amount; ++i) {
      if (item.baseItemId) {
        ...
        if (target !== null) this.removeIndependentItem(target);
      } else {
